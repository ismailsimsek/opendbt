...
from clients.jinja import (get_rendered)
...
class ModelParser(SimpleSQLParser[ModelNode]):
    ...
    def render_update(self, node: ModelNode, config: ContextConfig) -> None:
        ...
        # if the static parser didn't succeed, fall back to jinja
            ...
            # jinja rendering has been run, now we can parse the sql
            self.sqlglot_parse(node)
    ...
    def sqlglot_parse(self, node: ModelNode) -> None:
        if not node.compiled_code:
            print(f"{node.name} sqlglot parsing! Node dont have compiled code!")
            return
        try:
            expressions = sqlglot.parse(node.compiled_code, read=self.root_project.credentials.type)
            if not expressions:
                print(f"{node.name} sqlglot parsing! sqlglot.parse returned none expressions!")
                return
            for expression in expressions:
                for ref in expression.find_all(exp.Table):
                    if len(ref.parts) > 1:
                        node.sources.append([ref.parts[0].name, ref.parts[1].name])
                    else:
                        node.refs.append(RefArgs(name=ref.name, package=None, version=None))
        except Exception as e:
            print(f"{node.name} sqlglot parsing! Exception {e}")
            pass
